...
<div class="flex-1 p-4 ml-[172px]">
        <div class="mb-4 flex justify-between items-center">
            <h2 class="text-2xl font-bold">Claim List</h2>
            <button id="" class="bg-orange-600 text-black px-4 py-2 rounded">Add Claim</button>
        </div>
    ...
    <div>
        <table id="table" class="min-w-full bg-black text-white rounded-xl">
            ...
            <tbody>
                <?php 
                $claimService = new ArticleService();
                $claims = $claimService->display();
                foreach ($claims as $claim): ?>
                <tr>
                <td class="py-2 px-4 border-b"><?= $claim['claimId']; ?></td>
                <td class="py-2 px-4 border-b"><?= $claim['description']; ?></td>
                <td class="py-2 px-4 border-b"><?= $claim['date']; ?></td>
                <td class="py-2 px-4 border-b"><?= $claim['clientId'] ?></td>
                <td class="py-2 px-4 border-b flex">
                <button class="bg-yellow-500 text-white px-2 py-1 rounded mr-2">Edit</button>
                <form action="../controllers/claimController.php" method="post">
                    <input type="hidden" name="action" value="deletClaim">
                    <input type="hidden" name="deleteClaimId" value="<?= $claim['claimId']; ?>">
    ...
    <div id="" class="fixed inset-0 bg-gray-900 bg-opacity-50 hidden  items-center flex justify-center">
        <div class="bg-white p-8 rounded shadow-lg w-96">
            <h2 class="text-2xl font-bold mb-4">Add Claim</h2>
            <form id="" action="../controllers/claimController.php" method="post" enctype="multipart/form-data">
                <div class="mb-4">
